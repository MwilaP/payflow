...
import { ArrowLeft, Download, Mail, FileText, DollarSign, ChevronDown, ChevronRight, Settings, Loader2 } from "lucide-react"
...
export default function PayrollHistoryDetailPage() {
  ...
  const handleEmailPayslips = async () => {
    ...
    try {
      ...
      const payrollService = await getPayrollHistoryService()
      const payslips = await payrollService.getPayslipsByPayrollHistory(id!)

      if (!payslips || payslips.length === 0) {
        toast({
          title: "No Payslips",
          description: "No payslips found for this payroll period.",
          variant: "destructive",
        })
        return
      }

      // Convert payslips to email data
      const emailData = emailService.payslipsToEmailData(payslips)

      // Send bulk emails
      const result = await emailService.sendBulkPayslips(emailData)
