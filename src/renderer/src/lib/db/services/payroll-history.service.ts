...
export const payrollHistoryService = (db: any) => ({
  ...
  async createPayrollRecord(payrollData: any): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to create payroll record: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async getPayrollRecordById(id: string): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to retrieve payroll record: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async updatePayrollRecord(id: string, updates: any): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to update payroll record: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async deletePayrollRecord(id: string): Promise<any> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to delete payroll record: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async getAllPayrollRecords(): Promise<PayrollHistory[]> {
    ...
    try {
      ...
      const filteredRecords = allRecords.filter(
        (record: PayrollHistory) => record._id && record._id.includes('payroll')
      )
      return filteredRecords
    } catch (error: any) {
      console.error('Error getting all payroll records:', error)
      throw new PayrollError(
        `Failed to retrieve payroll records: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async getPayrollRecordsByEmployee(employeeId: string): Promise<PayrollHistory[]> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to retrieve employee payroll records: ${error?.message || 'Unknown error'}`
      )
    }
  },

  // Get payroll records by date range
  async getPayrollRecordsByDateRange(
    startDate: string,
    endDate: string
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to retrieve payroll records by date range: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async getPayrollRecordsByStatus(status: string): Promise<PayrollHistory[]> {
    ...
    try {
      ...
      throw new PayrollError(
        `Failed to retrieve payroll records by status: ${error?.message || 'Unknown error'}`
      )
    }
  },
  ...
  async processPayroll(id: string, processedBy?: string): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      await logTransaction(
        ...
        id,
        'process',
        `Payroll status changed from ${currentRecord.status} to processing`,
        updates
      )
      return result
    } catch (error: any) {
      console.error(`Error processing payroll record ${id}:`, error)
      const payrollError = new PayrollError(
        `Failed to process payroll (current status: ${error.currentStatus || 'unknown'}): ${error.message || 'Unknown error'}`
      )
  ...
  async completePayroll(id: string): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      await logTransaction(
        ...
        id,
        'complete',
        `Payroll status changed from ${currentRecord.status} to completed`,
        updates
      )
      return result
    } catch (error: any) {
      ...
      const payrollError = new PayrollError(
        `Failed to complete payroll (current status: ${error.currentStatus || 'unknown'}): ${error.message || 'Unknown error'}`
      )
      payrollError.currentStatus = error.currentStatus
      throw payrollError
    }
  ...
  async cancelPayroll(id: string, cancelNotes: string): Promise<PayrollHistory | null> {
    ...
    try {
      ...
      await logTransaction(
        ...
        id,
        'cancel',
        `Payroll status changed from ${currentRecord.status} to cancelled`,
        { reason: cancelNotes }
      )
      return result
    } catch (error: any) {
      ...
      const payrollError = new PayrollError(
        `Failed to cancel payroll (current status: ${error.currentStatus || 'unknown'}): ${error.message || 'Unknown error'}`
      )
      payrollError.currentStatus = error.currentStatus
      throw payrollError
    }
  ...
  async getPayslipsByPayrollHistory(payrollHistoryId: string): Promise<Payslip[]> {
    ...
    try {
      ...
      const employeeIdsInPayroll = new Set(
        payrollHistory.items.map((item: { employeeId: string }) => item.employeeId)
      )
      const employees = allEmployees.filter((emp: { _id: string }) =>
        employeeIdsInPayroll.has(emp._id)
      )
      ...
      for (const employee of employees) {
        try {
          ...
          const payrollStructure = await payrollStructureService.getPayrollStructureById(
            employee.payrollStructureId
          )
          if (!payrollStructure) {
            console.warn(
              `Payroll structure ${employee.payrollStructureId} not found for employee ${employee._id}`
            )
            continue
          }
  ...
  async bulkProcessPayroll(ids: string[], processedBy?: string): Promise<PayrollHistory[]> {
    ...
    try {
      ...
      const validPayrolls = payrolls.filter(
        (payroll): payroll is PayrollHistory => payroll !== null
      )
      ...
      validPayrolls.forEach((payroll) => {
        try {
          ...
          throw new PayrollError(
            `Payroll ${payroll._id}: ${error.message || 'Invalid status transition'}`
          )
        }
      })
      ...
      await Promise.all(
        updatedPayrolls.map((payroll) =>
          logTransaction(
            ...
            payroll._id,
            'bulk-process',
            `Payroll status changed from ${payroll.status} to processing`,
            { processedBy: processedBy || 'System' }
          )
        )
      )
